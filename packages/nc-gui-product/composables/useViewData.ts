...
export function useViewData(
  ...
  async function loadGalleryData() {
    ...
    galleryData.value = isPublic.value
      ...
      : await $api.dbView.dbViewGalleryRead(viewMeta.value.id)
  }
  ...
  async function loadFormView() {
    if (!viewMeta?.value?.id) return
    try {
      const { columns, ...view } = await $api.dbView.dbViewFormRead(viewMeta.value.id)
      let order = 1
  ...
  async function updateFormView(view: FormType | undefined) {
    ...
    try {
      await $api.dbView.dbViewFormUpdate(viewMeta.value.id, view)
    } catch (e: any) {
