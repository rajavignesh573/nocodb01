...
export const useViewsStore = defineStore('viewsStore', () => {
  ...
  const loadViews = async ({
    ...
    if (tableId) {
      ...
      const response = (await $api.dbView.dbViewList(tableId)).list as ViewType[]
      if (response) {
        viewsByTable.value.set(
          tableId,
    ...
    try {
      switch (payload.type) {
        case _ViewTypes.GRID:
          return await $api.dbView.dbViewGridCreate(view.fk_model_id, payload)

        case _ViewTypes.GALLERY:
          payload.fk_cover_image_col_id = (view.view as GalleryType)?.fk_cover_image_col_id || null

          return await $api.dbView.dbViewGalleryCreate(view.fk_model_id, payload)

        case _ViewTypes.FORM:
          return await $api.dbView.dbViewFormCreate(view.fk_model_id, payload)
        ...
        case _ViewTypes.KANBAN:
          ...
          return await $api.dbView.dbViewKanbanCreate(view.fk_model_id, payload)

        case _ViewTypes.MAP:
          payload.fk_geo_data_col_id = (view.view as MapType)?.fk_geo_data_col_id || null

          return await $api.dbView.dbViewMapCreate(view.fk_model_id, payload)
        ...
        case _ViewTypes.CALENDAR:
          ...
          return await $api.dbView.dbViewCalendarCreate(view.fk_model_id, payload)
      }
    } catch (e: any) {
      console.error(e)
  ...
  const setCurrentViewExpandedFormMode = async (viewId: string, mode: 'field' | 'attachment', columnId?: string) => {
    ...
    try {
      if (isUIAllowed('viewCreateOrEdit')) {
        await $api.dbView.dbViewUpdate(viewId, {
