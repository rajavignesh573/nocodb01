...
export const useBase = defineStore('baseStore', () => {
  ...
  async function loadProjectMetaInfo(force?: boolean) {
    if (!baseMetaInfo.value || force) {
      baseMetaInfo.value = await api.base.baseMetaGet(base.value.id!, {})
    }
  }
  ...
  async function loadTables() {
    if (base.value.id) {
      ...
      //   await api.dbTable.dbTableList(base.value.id, {
  ...
  async function loadProject(_withTheme = true, forcedId?: string) {
    ...
    if (baseType.value === 'base') {
      try {
        ...
        sharedProject.value = await api.base.baseRead(baseData.base_id!)
      } catch (e: any) {
  ...
  async function updateProject(data: Partial<BaseType>) {
    ...
    if (data.meta && typeof data.meta === 'string') {
      await api.base.baseUpdate(baseId.value, data)
    } else {
              await api.base.baseUpdate(baseId.value, { ...data, meta: stringifyProp(data.meta) })
    }
...
*/
  ...
  async function hasEmptyOrNullFilters() {
    return await api.base.baseHasEmptyOrNullFilters(baseId.value)
  }
