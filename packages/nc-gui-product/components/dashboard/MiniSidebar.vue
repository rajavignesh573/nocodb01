...
import ProductMatcher from '~/components/ProductMatcher.vue'
...
const { activeWorkspaceId, isWorkspaceSettingsPageOpened, isIntegrationsPageOpened, isChatPageOpened, isProductPageOpened, isWorkspacesLoading } =
  storeToRefs(workspaceStore)

const { navigateToWorkspaceSettings, navigateToIntegrations: _navigateToIntegrations, navigateToChat: _navigateToChat, navigateToProduct } = workspaceStore
...
// Product Matcher functionality
const isProductMatcherOpen = ref(false)

const openProductMatcher = () => {
  isProductMatcherOpen.value = true
}
...
<template>
    ...
    <div class="flex flex-col items-center">
      ...
      <template v-if="!isMobileMode">
        ...
        <DashboardMiniSidebarItemWrapper v-if="isUIAllowed('workspaceSettings') || isUIAllowed('workspaceCollaborators')">
         ...
         <DashboardMiniSidebarItemWrapper v-if="isUIAllowed('workspaceChat')">
           <NcTooltip
             :title="isEeUI ? `${$t('objects.workspace')} ${$t('general.chat')}` : $t('general.chat')"
             placement="right"
             hide-on-click
             :arrow="false"
           >
         ...
         <DashboardMiniSidebarItemWrapper>
           <NcTooltip
             :title="isEeUI ? `${$t('objects.workspace')} ${$t('general.product')}` : $t('general.product')"
             placement="right"
             hide-on-click
             :arrow="false"
           >
             ...
             >
               <div
                 class="nc-mini-sidebar-btn"
                 :class="{
                   active: isProductPageOpened,
                 }"
               >
                 <GeneralIcon icon="ncPackage" class="h-4 w-4" />
               </div>
             </div>
           </NcTooltip>
         </DashboardMiniSidebarItemWrapper>
         ...
         <DashboardMiniSidebarItemWrapper>
           ...
           >
             <div
               ...
               @click="openProductMatcher"
             >
               <div
                 class="nc-mini-sidebar-btn"
                 :class="{
                   active: isProductMatcherOpen,
                 }"
               >
                 <GeneralIcon icon="ncProduct" class="h-4 w-4" />
               </div>
             </div>
           </NcTooltip>
         </DashboardMiniSidebarItemWrapper>
  ...
  <NcModal
    v-model:visible="isProductMatcherOpen"
    :title="'Product Matcher'"
    :width="1200"
    :closable="true"
    :mask-closable="true"
    class="nc-product-matcher-modal"
  >
