...
async function onSubmit() {
  ...
  if (isValid && form.type) {
    ...
    try {
      ...
      switch (form.type) {
        case ViewTypes.GRID:
          data = await api.dbView.dbViewGridCreate(tableId.value, form)
          break
        case ViewTypes.GALLERY:
          data = await api.dbView.dbViewGalleryCreate(tableId.value, form)
          break
        case ViewTypes.FORM:
          data = await api.dbView.dbViewFormCreate(tableId.value, {
            ...form,
            ...getDefaultViewMetas(ViewTypes.FORM),
          })
          break
        case ViewTypes.KANBAN:
          data = await api.dbView.dbViewKanbanCreate(tableId.value, form)
          break
        case ViewTypes.MAP:
          data = await api.dbView.dbViewMapCreate(tableId.value, form)
          break
        case ViewTypes.CALENDAR:
          data = await api.dbView.dbViewCalendarCreate(tableId.value, {
            ...form,
