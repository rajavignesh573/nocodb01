...
const [useProvideViewColumns, useViewColumns] = useInjectionState(
  (
    ...
    const loadViewColumns = async () => {
      ...
      if (view.value?.id) {
        const data = (isPublic ? meta.value?.columns : (await $api.dbViewColumn.dbViewColumnList(view.value.id)).list) as any[]
    ...
    const showAll = async (ignoreIds?: any) => {
      ...
      if (view?.value?.id) {
        if (ignoreIds) {
          await $api.dbView.dbViewShowAllColumn(view.value.id, {
            ignoreIds,
          })
        } else {
          await $api.dbView.dbViewShowAllColumn(view.value.id)
    ...
    const hideAll = async (ignoreIds?: any) => {
      ...
      if (view?.value?.id) {
        if (ignoreIds) {
          await $api.dbView.dbViewHideAllColumn(view.value.id, {
            ignoreIds,
          })
        } else {
          await $api.dbView.dbViewHideAllColumn(view.value.id)
    ...
    const updateGridViewColumn = async (id: string, props: Partial<GridColumnReqType>, undo = false) => {
      ...
      try {
        ...
        if (!isPublic.value && isUIAllowed('viewFieldEdit') && gridViewCols.value[id]?.id) {
          await $api.dbView.dbViewGridColumnUpdate(gridViewCols.value[id].id as string, {
            ...props,
          })
        }
