...
function onWindowResize(e?: any): void {
  ...
  if (!e && isLeftSidebarOpen.value && sideBarSize.value && !sideBarSize.value.current && !isMobileMode.value) {
...
function onResize(widthPercent: any) {
  if (isMobileMode.value || widthPercent === undefined || widthPercent === null) return
  ...
  <div class="h-full flex items-stretch">
    ...
    >
      ...
      <Splitpanes
        ...
        @resize="(event: any) => event[0]?.size !== undefined && onResize(event[0].size)"
      >
        <Pane
          min-size="15%"
