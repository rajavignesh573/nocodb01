...
export const useTablesStore = defineStore('tablesStore', () => {
  ...
  const loadProjectTables = async (baseId: string, force = false) => {
    ...
    const tables = await api.dbTable.dbTableList(baseId, {
  ...
  const updateTable = async (table: TableType, undo?: boolean) => {
    ...
    try {
      await $api.dbTable.dbTableUpdate(table.id as string, {
      ...
      // update metas
      const newMeta = await $api.dbTable.dbTableRead(table.id as string)
  ...
  const loadTableMeta = async (tableId: string) => {
    try {
      const meta = await $api.dbTable.dbTableRead(tableId as string)
      baseTables.value.set(
        meta.base_id!,
