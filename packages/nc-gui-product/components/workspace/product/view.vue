...
const handleMatchToggle = async (option: any) => {
  if (option.selected) {
    ...
    await rejectMatch(option.id)
  } else {
    // If not matched, match it
    await confirmMatch(option.id)
  }
}
    ...
    <div class="flex gap-2 items-center min-w-0 p-2 h-[var(--topbar-height)] border-b-1 border-gray-200">
      ...
      <SmartsheetTopbarCmdK v-if="!isNewSidebarEnabled" />
       ...
       <div class="w-64/100 bg-white">
            ...
            >
              <!-- Single Row Layout: [Image 1] [Image 2] [Button] [Details 1] [Details 2] -->
              <div class="flex items-center gap-6">
                ...
                <div class="flex flex-col items-center justify-center min-w-0">
                  <button
                    ...
                    :class="[
                      'match-button flex items-center justify-center rounded-full transition-all duration-200 shadow-sm',
                      option.selected
                        ? 'bg-green-500 hover:bg-green-600 text-white shadow-green-200'
                        : 'bg-gray-200 hover:bg-gray-300 text-gray-600 shadow-gray-200'
                    ]"
                    :title="option.selected ? 'Unmatch products' : 'Match products'"
                  >
                    <GeneralIcon 
                      :icon="option.selected ? 'check' : 'plus'" 
                      class="h-5 w-5"
                    />
                  </button>
                  <span 
                    :class="[
                      ...
                      option.selected ? 'text-green-600' : 'text-gray-500'
                    ]"
                  >
                    {{ option.selected ? 'Matched' : 'Match' }}
                  </span>
                </div>
