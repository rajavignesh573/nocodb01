...
export const useBases = defineStore('basesStore', () => {
  ...
  async function getBaseUsers({ baseId, searchText, force = false }: { baseId: string; searchText?: string; force?: boolean }) {
    ...
    const response: any = await api.auth.authBaseUserList(baseId, {
      query: {
        query: searchText,
      },
    } as RequestParams)
  ...
  const createProjectUser = async (baseId: string, user: User) => {
          await api.auth.authBaseUserAdd(baseId, user as ProjectUserReqType)
  }

  const updateProjectUser = async (baseId: string, user: User) => {
          await api.auth.authBaseUserUpdate(baseId, user.id, user as ProjectUserReqType)
  ...
  const removeProjectUser = async (baseId: string, user: User) => {
          await api.auth.authBaseUserRemove(baseId, user.id)
  }
  ...
  const loadProjects = async (page: 'recent' | 'shared' | 'starred' | 'workspace' = 'recent', _?: string) => {
    ...
    if (route.value.params.typeOrId === 'base' && route.value.params.baseId) {
      try {
        ...
        const base: BaseType = await $api.base.baseRead(base_id)
    ...
    try {
      const { list } = await $api.base.baseList({
  ...
  const loadProject = async (baseId: string, force = false) => {
    try {
      ...
      const _project = await api.base.baseRead(baseId)
  ...
  const updateProject = async (baseId: string, baseUpdatePayload: BaseType) => {
    ...
    await api.base.baseUpdate(baseId, baseUpdatePayload)
  ...
  const createProject = async (basePayload: {
    ...
    const result = await api.base.baseCreate(
      {
  ...
  const deleteProject = async (baseId: string) => {
    await api.base.baseDelete(baseId)
  ...
  async function getProjectMetaInfo(baseId: string) {
    return await api.base.baseMetaGet(baseId!, {})
  }
    ...
    try {
      await Promise.all(
        updatedBasesOrder.map(async (base) => {
          await api.base.baseUpdate(base.id!, { order: base.order })
        }),
      )
    } catch (e: any) {
      message.error(await extractSdkResponseErrorMsg(e))
