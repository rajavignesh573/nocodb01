...
const [useProvideCalendarViewStore, useCalendarViewStore] = useInjectionState(
  (
    ...
    async function loadCalendarMeta() {
      ...
      try {
        const res = isPublic.value ? (sharedView.value?.view as CalendarType) : await $api.dbView.dbViewCalendarRead(viewMeta.value.id)
    ...
    async function updateCalendarMeta(updateObj: Partial<CalendarType>) {
      ...
      try {
        await $api.dbView.dbViewCalendarUpdate(viewMeta.value.id, {
          ...updateObj,
