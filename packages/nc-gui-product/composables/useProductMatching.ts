...
export function useProductMatching() {
  ...
  const filteredProducts = computed(() => {
    ...
    if (filters.value.status) {
      filtered = filtered.filter(product => {
        const productMatches = Array.isArray(matches.value) ? matches.value.filter(match => match.local_product_id === product?.id) : []
        return productMatches.some(match => match.status === filters.value.status)
      })
    }

    // Sort - ensure filtered is still an array before spreading
    ...
    if (filters.value.orderBy === 'price') {
      ...
      filtered = [...filtered].sort((a, b) => {
        const aMatches = Array.isArray(matches.value) ? matches.value.filter(match => match.local_product_id === a?.id) : []
        const bMatches = Array.isArray(matches.value) ? matches.value.filter(match => match.local_product_id === b?.id) : []
  ...
  const productSelectionOptions = computed(() => {
    ...
    // Ensure candidates.value is an array
      ...
      for (const candidate of sourceCandidates) {
        ...
        if (selectedProduct.value && candidate) {
          options.push({
            ...
            priceDelta: candidate.score || 0, // This should be calculated based on price difference
  ...
  const loadProducts = async () => {
    try {
      ...
      // Ensure result.items is always an array (backend returns 'items')
  ...
  const loadCandidates = async (productId: string) => {
    try {
      ...
      // Ensure result.items is always an array
