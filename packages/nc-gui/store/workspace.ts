...
import { navigateToBlankTargetOpenOption } from '~/utils/urlUtils'
...
export const useWorkspace = defineStore('workspaceStore', () => {
  ...
  const isChatPageOpened = computed(() => {
    console.log('Current route name:', route.value.name)
    console.log('Current route path:', route.value.path)
    // Use the same pattern as integrations
    return route.value.name === 'index-typeOrId-chat'
  })

  const isProductPageOpened = computed(() => route.value.name === 'index-typeOrId-product')

  const isProductMatcherPageOpened = computed(() => route.value.name === 'index-typeOrId-product-matcher')
  ...
  const addToFavourite = async (baseId: string) => {
    try {
      ...
      await $api.base.baseUserMetaUpdate(
        baseId,
        {
          starred: true,
        },
  ...
  const navigateToChat = async (_?: string, cmdOrCtrl?: boolean, query: Record<string, string> = {}) => {
    console.log('navigateToChat called with:', { cmdOrCtrl, query })
    if (cmdOrCtrl) {
      await navigateTo(
        { path: '/nc/chat', query },
        {
      ...
      console.log('Navigating to /nc/chat')
      // Use the same pattern as integrations
      await navigateTo({ path: '/nc/chat', query })
    }
  }

  const navigateToProduct = async (_?: string, cmdOrCtrl?: boolean, query: Record<string, string> = {}) => {
    if (cmdOrCtrl) {
      await navigateTo(
        { path: '/nc/product', query },
        {
          open: navigateToBlankTargetOpenOption,
        },
      )
    } else {
      await navigateTo({ path: '/nc/product', query })
    }
  }

  const navigateToProductMatcher = async (_?: string, cmdOrCtrl?: boolean, query: Record<string, string> = {}) => {
    if (cmdOrCtrl) {
      await navigateTo(
        { path: '/nc/product-matcher', query },
        {
          open: navigateToBlankTargetOpenOption,
        },
      )
    } else {
      await navigateTo({ path: '/nc/product-matcher', query })
    }
  }
  ...
  return {
    ...
    navigateToChat,
    navigateToProduct,
    navigateToProductMatcher,
    isIntegrationsPageOpened,
    navigateToIntegrations,
    isChatPageOpened,
    isProductPageOpened,
    isProductMatcherPageOpened,
    isFeedPageOpened,
    deletingWorkspace,
    isWorkspacesLoading,
    upgradeWsDlg,
