...
const [setup, use] = useInjectionState(() => {
  ...
  const loadUsers = async (page = currentPage.value, limit = currentLimit.value) => {
    try {
      ...
      const response: any = await api.auth.authBaseUserList(base.value?.id, {
        query: {
          limit,
          offset: (page - 1) * limit,
  ...
  const inviteUser = async (user: Partial<User>) => {
    ...
    try {
      ...
      res = await api.auth.authBaseUserAdd(base.value.id, {
        ...user,
    ...
    try {
      await Promise.all(
        _editedUsers
          ...
          .map(async (user) => {
            await api.auth.authBaseUserUpdate(base.value!.id!, user.id, {
              email: user.email,
              roles: user.roles,
              base_id: base.value.id,
              baseName: base.value.title,
      ...
      await Promise.all(
        _editedUsers
          .filter((user) => user.roles === 'No access')
          .map(async (user) => {
            await api.auth.authBaseUserRemove(base.value!.id!, user.id)
          }),
      )
